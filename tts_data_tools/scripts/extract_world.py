...
from .save_features import save_lf0, save_vuv, save_sp, save_ap
...
def extract_world(file_ids, wav_dir):
...
def process(wav_dir, id_list, out_dir, calculate_normalisation, normalisation_of_deltas):
    ...
    lf0_list, vuv_list, sp_list, ap_list = extract_world(file_ids, wav_dir)
    ...
    if calculate_normalisation:
        ...
        calculate_mvn_parameters(sp_list, out_dir, feat_name='ap', deltas=normalisation_of_deltas)
        calculate_mvn_parameters(ap_list, out_dir, feat_name='ap', deltas=normalisation_of_deltas)


def main():
    parser = argparse.ArgumentParser(
        description="Script to extract WORLD parameters from wav files.")
